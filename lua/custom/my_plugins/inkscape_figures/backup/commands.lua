...
function M.create(opts)
    ...
    if not ok then
        api.nvim_err_writeln(
            "inkscape_figures: failed to copy template.svg → " .. err
        )
        return
    end
    ...
    -- display the command in the command line for debugging
    api.nvim_echo({ { "Running: " .. cmd, "Normal" } }, false, {})
    ...
    local handle = uv.spawn("sh", {
        ...
        if code ~= 0 then
            api.nvim_err_writeln(
                "inkscape_figures: failed to compile " .. name .. ": " .. code
            )
        end
    end)
end
...
function M.edit(opts)
    ...
    for i, f in ipairs(files) do
        choices[#choices + 1] =
            string.format("%2d. %s", i, vim.fn.fnamemodify(f, ":t:r"))
    end
