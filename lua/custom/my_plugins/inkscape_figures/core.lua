...
local function insert_markup(markup_code)
    local cursor_pos = vim.api.nvim_win_get_cursor(0)
    local markup_lines = vim.split(markup_code, "\n")

    vim.api.nvim_buf_set_lines(0, cursor_pos[1], cursor_pos[1], false, markup_lines)
    vim.api.nvim_win_set_cursor(0, { cursor_pos[1] + #markup_lines, cursor_pos[2] })
end

local function start_inkscape_job(filepath, filename, filetype)
    ...
    vim.fn.jobstart(cmd, {
        on_exit = function(_, exit_code, _)
            vim.schedule(function()
                if exit_code == 0 then
                    -- Only run LaTeX/PDF conversion if we are in a LaTeX file
                    if filetype == "tex" then
                        vim.notify("Inkscape closed. Converting " .. filename .. " to PDF/LaTeX...", vim.log.levels.INFO)
                        M.convert_to_pdf_latex(filename)
                    else
                        vim.notify("Inkscape closed. Markdown figure saved.", vim.log.levels.INFO)
                    end
                else
...
local function create_figure(title, opts)
    opts = opts or {}
    local ft = vim.bo.filetype
    local config = utils.get_config(ft, opts)
    ...
    local base_filename = string.sub(filename, 1, #filename - 4)
    local markup_code = utils.generate_markup(base_filename, title, ft, opts)
    insert_markup(markup_code)

    start_inkscape_job(filepath, base_filename, ft)
end

local function edit_figure(figure_name, opts)
    opts = opts or {}
    local ft = vim.bo.filetype
    local config = utils.get_config(ft, opts)
    local filepath = config.figure_dir .. figure_name .. ".svg"
    ...
    start_inkscape_job(filepath, figure_name, ft)
end

local function convert_to_pdf_latex(filename)
    -- Note: We force 'tex' here since this is only called for LaTeX
    local config = utils.get_config("tex") 
    local figure_dir = config.figure_dir
...
-- Helper to extract figure name based on filetype regex
local function extract_figure_name(text, ft)
    if ft == "tex" then
        return string.match(text, "\\incfig{(.-)}")
    elseif ft == "markdown" then
        -- Matches standard markdown image syntax: ![...](images/filename.svg)
        return string.match(text, "!%[.-%]%(.-/([^/]+)%.svg%)")
    end
    return nil
end

function M.handle_normal_mode(opts)
    local ft = vim.bo.filetype
    local current_line = vim.api.nvim_get_current_line()
    local in_figure = extract_figure_name(current_line, ft)

    if in_figure then
        M.edit_figure(in_figure, opts)
    elseif string.match(current_line, "^%s*$") then
        ...
        if figure_name and figure_name ~= "" then
            M.create_figure(figure_name, opts)
        end
    else
...
function M.handle_visual_mode(opts)
    local ft = vim.bo.filetype
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<Esc>", true, false, true), "x", false)

    -- ... existing visual extraction logic ...

    local in_figure = extract_figure_name(selected_text, ft)
...
function M.handle_insert_mode(opts)
    local current_line = vim.api.nvim_get_current_line()
